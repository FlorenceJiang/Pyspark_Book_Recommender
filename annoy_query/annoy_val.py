...
def annoy_val(spark, dirname, rank, regParam, k, random_seed, nns):
    ...
    ground_truth = val_data.filter(col('rating') >= 3.0).groupBy('user_id').agg(
        collect_set('book_id').alias('true_books'))
    ...
    print(
        f'Total evaluation over {dirname}/{rank}_{regParam}_model using Annoy takes {str(datetime.timedelta(seconds=end_time - start_time))}')
